...
function TrainingName() {
  ...
  const [All_Training_Name, setAll_Training_Name] = useState([]);
  ...
  const Add_Training_Name = async () => {
    try {
      const resp = await axios.post(`${base_url}/api/lookup/SaveLookup`, {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "training_name",
        lookup_value: Training_Name.training_name,
        parent_lookup_value:Training_Name.training_category
      });
      if (resp.status === 200) {
        toast.success("Training Name Saved", { autoClose: 2000 });
      }
    } catch (error) {
      console.log(error);
    }
  ...
  // Pagination
      ...
      setAll_Training_Name(resp.data.data);
...
{
    setTraining_Name({...Training_Name,training_name:item.lookup_value,training_category:item.parent_lookup_value})
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input
                ...
                value={Training_Name.training_name}
                onChange={(e) =>
                  setTraining_Name({
                    ...Training_Name,
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded-md p-2 h-10"
                value={Training_Name.training_category}
                onChange={(e) =>
                  setTraining_Name({
                    ...Training_Name,
                    training_category: e.target.value,
                  })
                }
              >
          ...
          <table className="min-w-full table-auto border border-gray-200">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="px-4 py-2 border">Training Category</th>
                <th className="px-4 py-2 border">Action</th>
              </tr>
            </thead>
            <tbody>
              {All_Training_Name.map((item, index) => (
