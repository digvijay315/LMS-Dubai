...
function TrainingType() {
  ...
  const Add_Training_Type = async () => {
    try {
      const resp = await axios.post(`${base_url}/api/lookup/SaveLookup`, {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "training_type",
        lookup_value: Training_Type.training_type,
      });
      if (resp.status === 200) {
        toast.success("Training Type Saved", { autoClose: 2000 });
      }
    } catch (error) {
      console.log(error);
    }
  ...
  {
      setTraining_Type({...Training_Type,training_type:item.lookup_value})
      ...
      <div className="flex-1 ml-64 p-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block mb-2 font-medium">Training Type</label>
              <input
                type="text"
                placeholder="Training Type"
                className="w-full border border-gray-300 rounded-md p-2"
                value={Training_Type.training_type}
                onChange={(e) =>
                  setTraining_Type({
                    ...Training_Type,
